...
; UpdateScroll:
;-------------------------------------------------------------------------------
; SCROLL_DIRECTION - tracks the direction of the screen scroll  \
; SCROLL_MOVING - flag to check when the screen starts moving
; SCROLL_COUNT_X - cycles through 0-7 horizontal intervals to scroll pixels
; SCROLL_COUNT_Y - cycles through 0-7 vertical intervals to scroll pixels
...
;-------------------------------------------------------------------------------
;-------------------------------------------------------------------------------
; <SCREEN2_MEM, >SCREEN2_MEM - reads switching screen backgrounds
;===============================================================================
; ; Draws up to 40 pixels (cpy #40)
;===============================================================================
#region "DrawUpBuffer"
DrawUpBuffer
...
;-------------------------------------------------------------------------------
; SCREEN1_MEM - screen one draws from 40,760 (19 lines to the left)
;===============================================================================
...
;-------------------------------------------------------------------------------
; SCREEN1_MEM - screen one draws from 39,799 (19 lines to the right)
;===============================================================================
...
; delta values
;-------------------------------------------------------------------------------
; MAP_X_POS = PARAM1, MAP_X_DELTA = PARAM3
;===============================================================================
; SCROLL_DIRECTION, #SCROLL_DOWN - checks how far we scrolled vertically
;===============================================================================
; MAP_Y_DELTA = and masked with #0011. Then MAP_Y_DELTA = PARAM4 
;===============================================================================
...
;        sta tile_counter                ; and reset the tile counter for the loop
        ...
        lda MAP2025_LINE_LOOKUP_LO,x        ; and store the address for that line in
...
@level2tilemap
        lda (ZEROPAGE_POINTER_2),y      ; Fetch the tile number in A
        tax
        lda TILE2_NUMBER_LOOKUP_LO,x     ; of the tile itself and store it in ZEROPAGE_POINTER_1
        sta ZEROPAGE_POINTER_1
...
@level2colors
        lda ATTRIBUTE2_MEM,x             ; copy attribute for tile x
...
@tileloop
        ...
        lda maptwoloaded
        bne @leveltworightmap 

        lda MAP2025_LINE_LOOKUP_LO,x
        sta ZEROPAGE_POINTER_2          ; store the map line address in
        lda MAP2025_LINE_LOOKUP_HI,x        ; ZEROPAGE_POINTER_2
        sta ZEROPAGE_POINTER_2 + 1
        jmp @skipalleymap

; Alley Map is loaded: maptwoloaded=1
@leveltworightmap
        lda ALLEYMAP_LINE_LOOKUP_LO,x
        sta ZEROPAGE_POINTER_2
        lda ALLEYMAP_LINE_LOOKUP_HI,x
...
@copyloop
        ...
        lda maptwoloaded
        bne @level2rightcolors

; Display issue: tax may need to also appear under
; @level2colors
        lda ATTRIBUTE_MEM,x             ; use it to fetch that characters attribute data
        jmp @skiplevel2rightcolors
...
@copyloop
        ...
        lda maptwoloaded
        bne @level2leftcolors


        lda ATTRIBUTE_MEM,x             ; lookup the attribute for that character
        jmp @skiplevel2leftcolors

@level2leftcolors
        lda ATTRIBUTE2_MEM,x             ; lookup the attribute for that character
